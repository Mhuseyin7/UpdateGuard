...
func Transition(job *domain.UpdateJob, next domain.UpdateState) error {
	if !allowed[job.State][next] { return fmt.Errorf("invalid update transition %s -> %s", job.State, next) }
	job.State = next
	return nil
}

func IsTerminal(s domain.UpdateState) bool { return s == domain.Success || s == domain.RolledBack || s == domain.RollbackFailed || s == domain.Cancelled }
