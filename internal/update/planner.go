...
type Plan struct { ServiceID, CurrentDigest, TargetDigest string; Steps []string; RequiresApproval bool; Warnings []string }
...
func BuildPlan(s domain.Service, current, target domain.Snapshot, verify domain.Verification) (Plan, error) {
	if current.Digest == "" { return Plan{}, fmt.Errorf("current image digest is required for rollback") }
	if target.Digest == "" { return Plan{}, fmt.Errorf("target image digest is required") }
	...
	if strings.HasSuffix(strings.Split(s.Image, ":")[len(strings.Split(s.Image, ":"))-1], "latest") { p.Warnings = append(p.Warnings, "floating tag: target is pinned to its resolved digest before deployment") }
	return p, nil
}
